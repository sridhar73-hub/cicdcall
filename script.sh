...
Please log-in to Testsigma to check Test Plan run results."
#**********************************
...
function checkTestPlanRunStatus(){
  ...
  do
    ...
    if [[ $EXECUTION_STATUS =~ "STATUS_IN_PROGRESS" ]]; then
      echo "Poll #$(($i+1)) - In progress... waiting $SLEEP_TIME seconds..."
      sleep $SLEEP_TIME
    elif [[ $EXECUTION_STATUS =~ "STATUS_CREATED" ]]; then
      echo "Poll #$(($i+1)) - Created... waiting $SLEEP_TIME seconds..."
      sleep $SLEEP_TIME  
      ...
      echo "Unexpected status. Please check run results."
    fi
  done
}
...
function saveFinalResponseToJUnitFile(){
  ...
  echo "JUNIT Report saved: $JUNIT_REPORT_FILE_PATH"
}
...
# ✅ Store Run ID in a file for later use
echo "$RUN_ID" > execution_id.txt
echo "Execution ID saved to execution_id.txt"
...
if [ ! $HTTP_STATUS -eq 200  ]; then
  ...
  echo "Number of maximum polls: $POLL_COUNT"
